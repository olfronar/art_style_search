...
CAPTION_PROMPT = (
    ...
    "- **Colors**: Dominant palette, color relationships, saturation, temperature, gradients.\n"
    "- **Composition**: Layout, focal points, balance, use of space, perspective, framing.\n"
    "- **Technique**: Medium (oil, watercolor, digital, etc.), brushwork or rendering style, line quality, level of "
    "detail, abstraction vs realism.\n"
...
async def _caption_single(
    ...
    *,
    ...
    """Caption a single image, using the disk cache when valid."""
...
async def caption_references(
    ...
    *,
    ...
    For each image, checks for a valid cached caption at ``{cache_dir}/{stem}.json``.
    A cache entry is valid when the image file's mtime matches the stored value.
    Cache misses are resolved by calling Gemini, throttled by *semaphore*.
    """
    tasks = [
        _caption_single(path, model=model, client=client, cache_dir=cache_dir, semaphore=semaphore)
