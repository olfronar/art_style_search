...
CAPTION_SYSTEM = (
    ...
    "[Art Style] skeleton discipline (describe HOW it is rendered, never NAME the genre):\n"
    "- Anti-name preamble — do NOT label the style by genre inside [Art Style]. Phrases like "
    "'3D CGI of X', 'cel-shaded anime', '{Artist}-style', 'watercolor illustration' are forbidden here. "
    "Describe the technique so precisely that the reader reconstructs the genre from the principles.\n"
    "- The meta-prompt's slot names below are slot DEFINITIONS — paraphrase the content in your own voice. "
    "Do not echo the slot headings verbatim as captioner output.\n"
    "- Cover six slots in 2-5 sentences each; tie-break SHORTER when the signal is the same. "
    "Let these six slots carry the [Art Style] block:\n"
    "  1. How to Draw: medium class (A/B/C/D/E) with 3-5 observable cues; construction order "
    ...
    "layers); edge softness (hard / feathered / graduated); key-fill-rim direction and temperature.\n"
    "  3. Color Principle: palette family in generic terms ('saturated complementary blues', 'muted earth "
    "tones with chromatic shadows'); value structure (key + contrast); saturation policy (uniform / "
    "selective chroma / muted).\n"
    ...
    "  5. Stylization Attitude: body-deformation policy (the heads-tall archetype itself lives in [Subject]); "
    "one signature rendering quirk; mood register in one clause.\n"
    "  6. Level of Detail: detail density (low-poly / mid / filigree); abstraction stance (graphic / "
    "illustrative / naturalistic).\n"
    "- [Art Style] length target: 600-1200 words (well above the 100-word anchor floor; concentrated on the "
    "six slots, no prose inflation).\n\n"
    ...
    "Say-once discipline (cross-section dedup):\n"
    "- Each observation belongs to EXACTLY ONE caption section. If a fact fits [Subject], do not also "
    "state it in [Color Palette], [Lighting], or [Composition]. Inside [Art Style], each observation "
    "belongs to exactly one skeleton slot — palette detail goes in slot 3 (Color Principle) alone; "
    "grain and material vocabulary in slot 4 (Surface & Texture) alone.\n"
    "- When an observation could arguably fit two sections, pick the MORE SPECIFIC one. Color temperature "
    "of the light goes in slot 2 (Shading & Light), not slot 3 (Color Principle). Character body ratios "
    "go in the [Subject] Proportions sub-block, not Stylization Attitude.\n"
    "- Cross-reference instead of restating. If a rule or observation was already established upstream, "
    "downstream sections refer to it by name ('consistent with the Cool Rim Rule', 'as noted in [Art "
    "Style] slot 4') rather than repeating the content. This already applies to audit blocks; it "
    "applies equally between ordinary caption sections.\n\n"
    "Laconic discipline (short over long when signal is equal):\n"
    "- Prefer compact comma-delimited visual tokens over prose sentences when enumerating observations. "
    "'cadmium red accent on the bow tie, mustard yellow suspenders, navy waistcoat' beats a three-"
    "sentence narration of the same three facts.\n"
    "- One observation per sentence. If a sentence carries two distinct observations, split it.\n"
    "- When the same observation can be stated in 8 words or 20, use 8. Adjective chains list at most "
    "3 modifiers before the noun; more than 3 is padding.\n"
    "- Do not restate mood across sections. A single clause of mood register in [Art Style] slot 5 is "
    "enough; do not repeat it in [Lighting & Atmosphere].\n\n"
    ...
    "Forbidden terms (never use — no exceptions, including inside any named-movement reference):\n"
    "  cartoon, cartoonish, stylised, stylized (as bare adjective), beautiful, epic, whimsical, charming, cinematic.\n"
    "Forbidden padding patterns (filler with zero signal — cut every instance):\n"
    "  'which showcases', 'a sense of', 'serves to evoke', 'intricate details', 'beautifully rendered', "
    "'a stunning example of', 'masterfully crafted', 'effortlessly conveys', 'with a touch of', "
    "'carefully composed', 'giving the impression of', 'adding depth to'."
)
...
CAPTION_PROMPT = (
    ...
    "[Art Style]: Shared style DNA — describe HOW the style is rendered, never NAME the genre. "
    "Cover six slots in 2-5 sentences each in your own voice: "
    ...
    "(5) Stylization Attitude (body-deformation + one signature quirk + mood register), "
    "(6) Level of Detail (density + abstraction stance). "
