...
_BASE_REQUIREMENTS = (
    ...
    "1. First section MUST be 'style_foundation' — IS the [Art Style] block content (verbatim canon the "
    "captioner pastes into every caption, and the image generator reads as the style descriptor).\n"
    ...
    "5. 'style_foundation.value' IS the [Art Style] canon — the declarative text the captioner pastes verbatim. "
    "Write third-person assertions about this style ('This style renders as...', 'Edges are...', "
    "'The palette pulls from...'), NOT directives to a captioner. Forbidden inside the value: imperative verbs "
    "(Begin, Write, Declare, Target, Avoid, Follow, Do not, Before), audit scaffolding (SLOT N, '- [ ]', "
    "MANDATORY), meta-references ('this block', 'each slot', 'REUSABLE DNA'), and word-count targets. "
    "The value covers five facets in order, each 2-4 declarative sentences: How to Draw (open with the literal "
    "'How to Draw:' marker — medium in plain observable vocabulary + line/edge policy; describe the **finished "
    "surface**, NOT construction steps, primitives, or fabrication), "
    ...
    "7. Be laconic: state each style assertion ONCE in style_foundation. Other sections describe per-image "
    "observations without restating the canon — duplication adds words without adding signal.\n"
)
...
def _expand_system() -> str:
    return (
        ...
        "- First section MUST be 'style_foundation'; its value IS the [Art Style] canon the captioner pastes "
        "verbatim into every caption's [Art Style] block AND the image generator reads as the style descriptor. "
        "Write third-person declarative assertions about this style ('This style renders as...', 'Edges are...', "
        "'The palette pulls from...'), NOT directives to a captioner. Forbidden inside the value: imperative verbs "
        "addressed to a reader, audit scaffolding (SLOT N, '- [ ]', MANDATORY), meta-references ('this block', "
        "'each slot', 'REUSABLE DNA'), and word-count targets. The value covers five facets in order, each 2-4 "
        "declarative sentences: "
        "(1) How to Draw (open with the literal 'How to Draw:' marker; medium identification in plain observable "
        "vocabulary matching what was analyzed + line/edge policy — describe the **finished surface**, NOT "
        "construction steps, primitives, or fabrication), "
        "(2) Shading & Light (layer stack + edge softness + key-fill-rim), "
        "(3) Color Principle (generic palette families + value + saturation; no image-specific hues), "
        "(4) Surface & Texture (grain + class-appropriate material vocabulary self-consistent with the medium), "
        "(5) Style Invariants (3-5 MUST/NEVER rules every image in this style obeys). "
        "Observations-vs-rules: every sentence must still be true of a DIFFERENT image in this style — no specific "
        "body parts, named objects, proper nouns, actual colors, or pose details. "
        "Forbid genre labels ('3D CGI of X', 'cel-shaded anime', '{Artist}-style') inside the value.\n"
