...
_BASE_REQUIREMENTS = (
    ...
    "5. 'style_foundation.value' MUST realize the six-slot [Art Style] SKELETON describing HOW the style "
    "is rendered (not NAMING the genre). Start with the literal 'How to Draw:' marker, then: Shading & Light "
    "(layer stack + edge softness + key-fill-rim), Color Principle (generic palette families + value + "
    "saturation), Surface & Texture (grain + class-appropriate material vocabulary), Stylization Attitude "
    "(body-deformation + signature quirk + mood register), and Level of Detail (density + abstraction "
    "stance). Genre labels like '3D CGI of X', 'cel-shaded anime', or '{Artist}-style' are forbidden inside "
    "[Art Style].\n"
...
def _expand_system() -> str:
    return (
        ...
        "inside every caption's [Art Style] block. It MUST realize the six-slot [Art Style] SKELETON: "
        ...
        "(5) Stylization Attitude (body-deformation + signature quirk + mood register), "
        "(6) Level of Detail (density + abstraction stance). "
        "Start with the literal 'How to Draw:' marker so validation finds it. Forbid genre labels "
        "('3D CGI of X', 'cel-shaded anime', '{Artist}-style') inside [Art Style] — describe HOW, not NAME.\n"
        ...
        "- [Art Style] targets 600-1200 words — concentrated on the six skeleton slots, no prose inflation. "
        "[Subject] can run 1000-2000 words when warranted. Ancillary caption sections usually land in the 150-400 word range.\n"
        ...
        "- STYLE-DNA PURITY: style_foundation / [Art Style] carries REUSABLE style DNA only. Its palette paragraph "
        "MUST name generic color FAMILIES ('saturated complementary blues', 'warm earth tones with chromatic shadows', "
        "'cadmium yellow + cerulean + magenta triad'). NEVER let image-specific nouns leak in ('crimson bow tie', "
        "'cadmium yellows of the chest', 'vibrant blue suspenders'). Image-specific palette facts belong in "
        "[Color Palette] or [Subject] — they are per-image observations, not style DNA. "
        "Additionally, NEVER label the style by genre ('3D CGI of X', 'cel-shaded anime', '{Artist}-style', "
        "'watercolor illustration') inside [Art Style] — describe the principles so the reader reconstructs the genre.\n"
