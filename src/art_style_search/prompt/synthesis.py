...
async def synthesize_templates(
    ...
    *,
    ...
    system = (
        ...
        "- The merged 'style_foundation.value' IS the [Art Style] canon — declarative text the captioner pastes "
        "verbatim. It MUST remain assertive canon (third-person claims about this style), NOT instructions to a "
        "captioner. Forbidden inside the merged value: imperatives (Begin, Write, Declare, Target, Avoid, Follow, "
        "Do not), audit scaffolding (SLOT N, '- [ ]', MANDATORY), meta-references ('this block', 'each slot'), and "
        "word-count targets. Preserve the five facets in order — How to Draw (open with the literal 'How to Draw:' "
        "marker; describe the **finished surface** — medium in plain observable vocabulary + line/edge policy — NOT "
        "construction steps, primitives, or fabrication), Shading & Light, Color Principle, Surface & Texture, "
        "Style Invariants — and the anti-name preamble "
        "forbidding genre labels. Silently dropping a facet, reintroducing per-image observations, or sliding into "
        "methodology framing is a regression.\n\n"
        "## Decision guidance\n"
        ...
        "[Subject] carries per-image weight including character proportions, 800-2000 words.\n\n"
        ...
        "- [ ] style_foundation.value reads as declarative canon (third-person assertions about this style), "
        "preserves all five facets (How to Draw / Shading & Light / Color Principle / Surface & Texture / Style Invariants), "
        "contains no imperatives to a captioner, no audit scaffolding, no word-count targets, and no genre labels\n\n"
        "Response format:\n"
        "{\n"
        '  "rationale": "...",\n'
        '  "template": {\n'
