...
async def synthesize_templates(
    ...
    *,
    ...
    system = (
        ...
        "- The merged 'style_foundation' MUST preserve the 5-slot [Art Style] SKELETON from the parents "
        "(How to Draw / Shading & Light / Color Principle / Surface & Texture / Style Invariants), the "
        "observations-vs-rules rule ([Art Style] holds RULES only, never per-image observations), and the "
        "anti-name preamble forbidding genre labels. Silently dropping a slot, introducing per-image "
        "observations, or re-introducing style-naming inside [Art Style] is a regression.\n\n"
        "## Decision guidance\n"
        ...
        "- [ ] style_foundation preserves all 5 [Art Style] skeleton slots, the observations-vs-rules rule, and the anti-name preamble\n\n"
        "Response format:\n"
        "{\n"
        '  "rationale": "...",\n'
        '  "template": {\n'
