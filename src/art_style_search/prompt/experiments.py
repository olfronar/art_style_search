...
def _experiment_system_prompt(
    *, current_template: PromptTemplate, response_format: str, is_first_iteration: bool = False
    ...
    base = (
        ...
        "4. style_foundation.value MUST realize the 5-slot [Art Style] SKELETON — RULES ONLY that apply to "
        "every image in this style, never per-image observations: "
        ...
        "(4) Surface & Texture (grain + material vocabulary that matches the observed medium), "
        "(5) Style Invariants (3-5 generative MUST/NEVER rules that every image in this style obeys). "
        "Start with the literal 'How to Draw:' marker so validation finds it. Genre labels "
        "('3D CGI of X', 'cel-shaded anime', '{Artist}-style') are forbidden inside [Art Style]. "
        "Character proportions belong in subject_anchor, not style_foundation. "
        "Refinements may reword slots but MUST keep all five present.\n"
    ...
    if is_first_iteration:
        context = (
            ...
            "First: [Art Style] (shared style DNA realized via the 5-slot skeleton — "
            "How to Draw / Shading & Light / Color Principle / Surface & Texture / Style Invariants — "
            "RULES ONLY, identical across captions, target 400-800 words). "
