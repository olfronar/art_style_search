...
def _experiment_system_prompt(
    *, current_template: PromptTemplate, response_format: str, is_first_iteration: bool = False
    ...
    base = (
        ...
        "1. First section MUST be 'style_foundation' — IS the [Art Style] block content (verbatim canon the captioner pastes into every caption).\n"
        ...
        "4. style_foundation.value IS the [Art Style] canon — the declarative text the captioner pastes verbatim "
        "into every caption's [Art Style] block AND the image generator reads as the style descriptor. "
        "Write third-person assertions about this style ('This style renders as...', 'Edges are...', "
        "'The palette pulls from...'), NOT directives to a captioner. "
        "Forbidden inside the value: imperative verbs addressed to a reader (Begin, Write, Declare, Target, "
        "Avoid, Follow, Do not, Before), audit scaffolding (SLOT N, '- [ ]', MANDATORY), meta-references "
        "('this block', 'each slot', 'REUSABLE DNA'), captioner-addressing preambles, and word-count targets. "
        "The value covers five facets in order — How to Draw (open with the literal 'How to Draw:' marker; "
        "describe the **finished surface** — medium in plain observable vocabulary + line/edge policy — NOT "
        "construction steps, primitives, or fabrication), "
        "Shading & Light, Color Principle, Surface & Texture, Style Invariants (3-5 MUST/NEVER rules) — "
        "each 2-4 declarative sentences describing this style. "
        "Genre labels ('3D CGI of X', 'cel-shaded anime', '{Artist}-style') are forbidden. "
        "Character proportions belong in subject_anchor, not style_foundation. "
        "Refinements may reword facets but MUST keep all five present and all assertive.\n"
        ...
        "6. LACONIC: state each style rule ONCE in style_foundation. Other sections reference rules by name. "
        "Pure verbatim restatement across sections is a regression.\n"
        "7. OBSERVATIONS-VS-RULES: style_foundation / [Art Style] holds RULES that apply to every image in this "
        "style. Per-image observations — specific body parts, named objects, proper nouns, actual colors, pose "
        "details — belong in [Subject], [Color Palette], [Composition], or [Lighting & Atmosphere], NEVER in "
        "[Art Style]. A sentence inside [Art Style] is well-formed only if it would still be true of a DIFFERENT "
        "image in the same style. The palette paragraph names generic color FAMILIES ('saturated complementary "
        "blues', 'warm earth tones') never image-specific nouns ('crimson bow tie'). Genre labels "
        "('3D CGI of X', 'cel-shaded anime', '{Artist}-style') are forbidden in [Art Style]. "
        "A meta-prompt that invites per-image contamination in [Art Style] is a regression.\n\n"
    ...
    if is_first_iteration:
        context = (
            ...
            "style_foundation.value IS the [Art Style] canon content — declarative style assertions the captioner "
            "pastes verbatim into every caption; ancillary sections layer per-image observations on top without restating the canon.\n\n"
            ...
            "First: [Art Style] (the style_foundation canon pasted verbatim across the 5 facets — "
            "How to Draw / Shading & Light / Color Principle / Surface & Texture / Style Invariants — "
            "declarative assertions about the style, identical across captions, typically 400-800 words). "
            "Second: [Subject] (per-image weight including character proportions, often 800-2000 words). "
            "Remaining sections (e.g. Color Palette, Composition, Lighting & Atmosphere) typically 150-400 words "
            "each — they are your optimization surface. Avoid adding 'Technique' or 'Textures' — they duplicate [Art Style].\n\n"
            "### Metrics\n"
...
def _expand_system(current_template: PromptTemplate, *, is_first_iteration: bool = False) -> str:
    return _experiment_system_prompt(
        ...
        response_format=(
            ...
            "- [ ] style_foundation.value realizes the 5-slot [Art Style] skeleton "
            "(How to Draw / Shading & Light / Color Principle / Surface & Texture / Style Invariants) "
            "and carries the observations-vs-rules + anti-name preamble\n"
            "- [ ] style_foundation holds RULES only — no specific body parts, named objects, or actual colors\n"
            ...
            "- [ ] style_foundation identifies the medium in plain observable vocabulary (no letter buckets) "
            "and its technique language stays consistent with the observed surface\n"
