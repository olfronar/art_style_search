...
_GEMINI_ANALYSIS_PROMPT = (
    ...
    "Describe the medium in 2-3 sentences using plain, observable vocabulary. What is it actually — "
    "hand-painted 2D, digital vector, stylized CGI render, photoreal CGI, a mix? Say what you SEE: "
    "is there visible brushwork or is the surface mathematically smooth? Are edges drawn with an outline "
    "or only formed by value contrast? Do forms read as painted onto a flat plane or as lit 3D volumes? "
    "Cite 3-5 observable cues that force this reading. Avoid genre labels and avoid committing to a "
    "single-word bucket — describe what's on the canvas. If references split across media, report primary + secondary.\n"
    ...
    "2. **Rendering technique**: Use vocabulary appropriate to what you observed in item 0. "
    "Describe line quality, edge treatment, level of detail vs abstraction, texture of the medium — "
    "pick terms that match the actual surface behavior (brushstroke / paper tooth / uniform fill / "
    "bezier curve / bevel / subdivision / ambient occlusion / rim light / PBR roughness / etc.). "
    "Don't mix vocabulary across incompatible media; let the observed surface drive word choice.\n"
...
_COMPILATION_PROMPT = (
    ...
    "    1. How to Draw: medium identification (what the surface actually is — hand-painted 2D, "
    "digital vector, stylized CGI render, photoreal CGI, mixed) + 3-5 observable cues; construction order "
    ...
    "- The captioner must identify the medium in plain, observable vocabulary inside style_foundation "
    "(e.g. 'hand-painted 2D with soft gradient shading and no visible brushwork'; 'stylized CGI render "
    "with beveled volumes and matte-plastic surfaces'; 'flat digital vector with hard silhouette edges'). "
    "Instruct the captioner to pick vocabulary that matches the observed surface behavior and to avoid "
    "mixing terms from incompatible media (don't reach for brushwork on a mathematically smooth CGI surface, "
    "or PBR roughness on a flat vector fill).\n"
