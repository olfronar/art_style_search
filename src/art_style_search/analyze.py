...
_GEMINI_ANALYSIS_PROMPT = (
    ...
    "0. **Medium class** (answer FIRST — this anchors every downstream decision). Pick EXACTLY ONE:\n"
    "    A. hand-drawn 2D — visible medium grain (paper tooth, watercolor bleeds, pigment pooling), hand-made line with pressure variation\n"
    "    B. vector / flat 2D — uniform solid fills, bezier-perfect curves, zero grain, hard silhouette edges\n"
    "    C. stylized 3D CGI — rounded bevels on every form, subdivision-smooth gradients, ambient occlusion in creases, rim light on focal subjects, matte plastic/fondant surfaces, no outlines\n"
    "    D. photoreal 3D CGI — PBR materials with roughness variation, physically plausible light falloff, DoF bokeh, subsurface scattering, film grain, HDRI reflections\n"
    "    E. mixed / 2.5D — 2D painted assets with parallax depth, or 3D model with hand-painted textures that fight the lighting\n"
    "   Cite 3-5 observable cues from the image(s) that force this class. If references split across classes, report primary + secondary.\n"
    ...
    "2. **Rendering technique**: Use vocabulary appropriate to the medium class you picked. "
    "For C/D use bevel / subdivision / ambient occlusion / rim light / specular / PBR. "
    "For A/B use brushstroke / paper tooth / ink bleed / pigment pooling / uniform fill / bezier curve. "
    "Describe line quality, edge treatment, level of detail vs abstraction, texture of the medium.\n"
...
_REASONING_ANALYSIS_PROMPT = (
    ...
    "4. **Technique principles**: What specific techniques define this style? Describe the HOW — "
    "construction order, line policy, shading stack, surface vocabulary, palette principle. "
...
_COMPILATION_PROMPT = (
    ...
    "- Be laconic: state each style rule ONCE in 'style_foundation'. Other sections reference rules by name "
    "(e.g. 'apply the line-weight rule from style_foundation') rather than restating them. "
    "Why: duplication inflates rendered word count without adding signal and increases the risk of inconsistent restatements.\n"
    "- 'style_foundation' is the SHARED DNA repeated verbatim inside every caption's [Art Style] block, driving "
    "style_consistency. Put invariant rules there; put per-image reasoning in the ancillary sections' instructions.\n"
    "- OBSERVATIONS-VS-RULES rule (the single most important rule for style_foundation): [Art Style] holds "
    "RULES that apply to every image in this style. Per-image observations — specific body parts, named objects, "
    "proper nouns, actual colors, pose details — belong in [Subject], [Color Palette], [Composition], or "
    "[Lighting & Atmosphere], NEVER in [Art Style]. A sentence inside [Art Style] is well-formed only if it "
    "would still be true of a DIFFERENT image in the same style.\n"
    "- 'style_foundation.value' MUST realize the 5-slot [Art Style] SKELETON — describe HOW the style "
    "is rendered, never NAME the genre. Start with the literal 'How to Draw:' marker. The 5 slots are:\n"
    "    1. How to Draw: medium class (A/B/C/D/E) + 3-5 observable cues; construction order "
    "(silhouette -> forms -> details); line policy (none / thin uniform / variable ink / painterly / crease-only).\n"
    "    2. Shading & Light: shading-layer stack in order (base -> AO -> midtones -> rim -> specular; omit "
    "absent layers); edge softness (hard / feathered / graduated); key-fill-rim direction and temperature. "
    "RULES ONLY — no specific body parts, no specific objects.\n"
    "    3. Color Principle: palette family in generic terms ('saturated complementary blues'); value "
    "structure (key + contrast); saturation policy (uniform / selective chroma / muted). RULES ONLY — no "
    "actual colors named.\n"
    "    4. Surface & Texture: grain type (paper tooth / film grain / stippling / zero grain); material "
    "vocabulary (matte plastic / fondant / PBR roughness / impasto / watercolor wash) — class-appropriate only. "
    "RULES ONLY — no specific objects.\n"
    "    5. Style Invariants: 3-5 MUST/NEVER rules that every image in this style obeys — e.g. "
    "'MUST: every character has exactly one exaggerated feature'; 'NEVER: more than three saturated hues in "
    "one frame'. Generative rules: they describe what a NEW image in this style would have to do.\n"
    "- Character proportions (heads-tall + archetype) belong in subject_anchor, NOT in style_foundation.\n"
    "- Anti-name preamble inside style_foundation: forbid genre labels in [Art Style] — "
    "'3D CGI of X', 'cel-shaded anime', '{{Artist}}-style', 'watercolor illustration' must not appear. "
    "Describe the technique so the reader reconstructs the genre from the principles.\n"
    "- 'subject_anchor.value' MUST contain a 'Proportions:' sub-block requiring the captioner to emit head-heights-tall "
    "(numeric, e.g. '3.2 heads tall') AND an archetype phrase — one of: chibi / stylized-youth / heroic / "
    "realistic-adult / elongated. This anchors character anatomy against generator drift toward default 7.5-head realism.\n"
    "- The captioner must classify the medium as EXACTLY ONE of: A hand-drawn 2D (brushstrokes, paper tooth, ink bleed); "
    "B vector / flat 2D (uniform fills, bezier curves); C stylized 3D CGI (bevels, ambient occlusion, rim light, "
    "matte plastic surfaces, no outlines); D photoreal 3D CGI (PBR, DoF, subsurface scattering); E mixed / 2.5D. "
    "Instruct the captioner to use class-appropriate vocabulary only; mention the observed class explicitly in style_foundation.\n"
    ...
    "targeting roughly 800-2000 words. [Art Style] holds generic RULES only (no image-specific observations), "
    "targeting 400-800 words across the 5 skeleton slots. Ancillary caption sections typically 150-400 words each. "
    ...
    "- [ ] style_foundation.value realizes the 5-slot [Art Style] skeleton (How to Draw / Shading & Light / "
    "Color Principle / Surface & Texture / Style Invariants), starting with the literal 'How to Draw:' marker\n"
    "- [ ] style_foundation contains the observations-vs-rules rule AND the anti-name preamble\n"
    "- [ ] style_foundation holds RULES only — no specific body parts, no named objects, no actual colors\n"
    ...
    "- [ ] style_foundation names the observed medium class (A/B/C/D/E) and uses class-appropriate vocabulary\n"
