...
_COMPILATION_PROMPT = (
    ...
    "    1. How to Draw (open with the literal marker 'How to Draw:' on its own sub-line). Describe the medium "
    "in plain observable vocabulary ('This style renders as hand-painted 2D with visible brushwork and paper "
    "tooth', or 'This style is a stylized CGI render with beveled subdivision-surface volumes and matte-plastic "
    "surfaces', or 'This style is a flat digital vector with hard silhouette edges and uniform fills'). Then "
    "state construction order (silhouette → forms → details) and line policy (none / thin uniform / variable ink "
    "/ painterly / crease-only) as assertions about what this style does.\n"
    "    2. Shading & Light — positive example of the right SHAPE (do NOT copy content — describe the observed "
    "style): 'Surfaces carry a four-layer stack: matte base, soft ambient-occlusion crease, warm-rim highlight, "
    "tiny specular pip on wet materials. Terminators are feathered at silhouettes and hard inside creases. Key "
    "light is warm and front-left; fill is cool sky-ambient.' Write declarative assertions of this shape about "
    "the analyzed style — layer stack, edge softness, key-fill-rim direction and temperature. No body parts, no "
    "specific objects.\n"
    "    3. Color Principle — positive example of SHAPE: 'This style locks to saturated complementary blues "
    "against warm flesh tones. Value sits in the mid-to-high register with crushed darks reserved for accent "
    "edges. Saturation peaks in midtones; shadows hue-shift cooler at equal-or-higher chroma rather than "
    "desaturating.' Name generic color FAMILIES, never actual image-specific hues.\n"
    "    4. Surface & Texture — positive example of SHAPE: 'Every surface reads as polished matte fondant or "
    "sculpted clay. Grain, film noise, and fabric weave are absent; implied texture is modeled as rounded "
    "grooves rather than painted. Material classes separate by specular breadth, not by texture noise.' Name "
    "class-appropriate material vocabulary that matches the observed medium; no specific objects.\n"
    "    5. Style Invariants — 3 to 5 MUST/NEVER claims that every image in this style obeys, each a standalone "
    "sentence. Example SHAPE: 'MUST bevel every structural edge; no 90-degree corner survives. MUST cool-shift "
    "shadows into saturated analogues. NEVER outline or cel-band any form. NEVER introduce photographic grain or "
    "strand-level hair.' These are generative rules — assertions about what a NEW image in this style does.\n"
