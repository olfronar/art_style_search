...
- Style consistency is measured via Jaccard word-overlap of [Art Style] blocks across captions and included in `composite_score` (8% weight). Canon pull-through is measured separately via two directional metrics in the compliance penalty: `compute_canon_fidelity` — **longest-common-substring ratio** (char-level via `difflib.SequenceMatcher` with whitespace-normalized inputs; score = LCS / len(canon)) between the caption's `[Art Style]` block and the meta-prompt's `style_foundation`. 1.0 = canon copied verbatim (the desired state); paraphrase breaks character-level contiguity so it scores <0.2; scrambled vocabulary that happens to share tokens can't fake a high score. Returns neutral 1.0 when canon <200 chars or caption `[Art Style]` block <50 chars. `compute_observation_boilerplate_purity` stays on token-trigram overlap (Jaccard-coverage is the right shape for contamination detection): 1.0 = observations stay clean, 0.0 = canon pasted into `[Subject]`/`[Color Palette]`/etc. `extract_style_canon` in `evaluate.py` pulls `style_foundation.value` from a rendered meta-prompt for the metric.
