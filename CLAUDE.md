...
- `prompt/initial.py` - Zero-step template generation via brainstorm→rank→expand (mirrors the iteration flow in `prompt/experiments.py`). `brainstorm_initial_sketches` (1 call, `max_tokens=40000`) produces `num_branches*2` short `InitialTemplateSketch` ideas; `rank_initial_sketches` (1 call, `max_tokens=1000`) reorders them with a skeptical-reviewer prompt; `expand_initial_sketches` (N parallel calls, `max_tokens=16000`) turns each top-N sketch into a full 1200-2500 word template. `propose_initial_templates` orchestrates all three and pads with empty placeholders for failed expansions so `_sanitize_initial_templates` can swap in the compiled fallback.
...
- `evaluate.py` - Per-image paired metrics dispatched via `asyncio.to_thread`, Gemini/xAI vision comparison, and caption compliance. `_VISION_SYSTEM` includes calibration examples for MATCH/PARTIAL/MISS across all three dimensions. `_PAIRWISE_COMPARE_PROMPT` requests per-aspect verdicts (style, color, subject, composition) alongside overall winner. `compute_caption_compliance` parses each caption once and returns `(CaptionComplianceStats, prose)` — `check_caption_compliance` and `compute_caption_compliance_stats` are thin wrappers. `compute_style_consistency` uses `parse_labeled_sections` for the `[Art Style]` block. `evaluate_images` returns `(scores, n_eval_failed)` with zero-score sentinels preserving index alignment; callers derive their own `completion_rate`.
...
- Meta-prompt is 1200-2500 words with 8-15 sections (4-8 sentences each). The FIRST section must be `style_foundation` (fixed style rules from StyleProfile) and the SECOND section must be `subject_anchor` (detailed subject-fidelity instructions). The first two caption output labels must be `[Art Style]` and `[Subject]`. These required anchors are enforced by `_REQUIRED_SECTION_ANCHORS` / `_REQUIRED_CAPTION_ANCHORS` tables in `prompt/_parse.py`.
