...
- **Vision medium** (2%): Per-image Gemini ternary agreement on the rendering medium — described in plain observable vocabulary (e.g. "hand-painted 2D", "stylized CGI render with beveled volumes"), no letter buckets. A medium mismatch is a style MISS in the vision judge, so this dim catches that failure explicitly. Higher = better.
...
- Style consistency is measured via Jaccard word-overlap of [Art Style] blocks across captions and included in `composite_score` (8% weight). Canon pull-through is measured separately via two directional metrics in the compliance penalty: `compute_canon_fidelity` (trigram overlap of caption `[Art Style]` block vs the meta-prompt's `style_foundation` — 1.0 = canon copied verbatim, the desired state) and `compute_observation_boilerplate_purity` (trigram overlap of the caption's per-image observation blocks vs `style_foundation` — 1.0 = observations stay clean, 0.0 = canon pasted into `[Subject]`/`[Color Palette]`/etc). `extract_style_canon` in `evaluate.py` pulls `style_foundation.value` from a rendered meta-prompt for the metric.
...
- Medium identification discipline: the captioner describes the rendering medium of every image in plain, observable vocabulary grounded in surface cues — "hand-painted 2D with soft gradient shading", "stylized CGI render with beveled volumes and matte-plastic surfaces", "digital vector with hard silhouette edges", etc. No letter buckets (A/B/C/D/E). The analyzer, captioner, and vision judge all use free-form medium language; the constraint is that technique words (brushwork vs subdivision vs PBR roughness) should match what's actually observable and not mix vocabulary across incompatible media. Medium cues are seeded by `_GEMINI_ANALYSIS_PROMPT` (`analyze.py`) and enforced in `CAPTION_SYSTEM` (`caption.py`); the vision judge uses matching calibration examples in `_VISION_SYSTEM` (`evaluate.py`). `CATEGORY_SYNONYMS` (`taxonomy.py`) carries `rendering_dimensionality` and `proportions` categories so the optimizer can target these failures directly.
