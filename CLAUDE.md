...
### Data + types
- `types.py` - Shared dataclasses (Caption, MetricScores, AggregatedMetrics, CaptionComplianceStats, StyleProfile, PromptTemplate, LoopState, KnowledgeBase, Hypothesis, ReviewResult, RunManifest, PromotionDecision, PromotionTestResult, ReplicatedEvaluation, etc.) plus the `RiskLevel = Literal["targeted", "bold"]` and `DirectionId = Literal["D1", "D2", "D3"]` aliases annotating directional-search fields. `AggregatedMetrics.summary_dict()` is derived from `fields(self)` so added metrics flow into serialization automatically. PromptTemplate includes `caption_sections` (ordered labeled output sections) and `caption_length_target`. `compliance_components_mean()` averages `CaptionComplianceStats` components with a divisor derived from `len(fields(CaptionComplianceStats))` — adding a sixth compliance metric updates the divisor automatically. `IterationResult`, `Hypothesis`, `ExperimentProposal`, and `RefinementResult` carry directional-search metadata: `direction_id` (`D1`/`D2`/`D3`), `direction_summary`, `failure_mechanism`, `intervention_type`, `risk_level` (`"targeted"` or `"bold"`), `expected_primary_metric`, `expected_tradeoff`, plus `changed_sections: list[str]` alongside the singular `changed_section`. `CategoryProgress` adds `last_mechanism_tried` / `last_confirmed_mechanism`.
...
- `prompt/review.py` - CycleResearcher-inspired `review_iteration` (skeptical reviewer → `strategic_guidance`). Phased into assess → synthesize → advise. Includes calibration standards (SIGNAL requires 2+ metrics, <0.01 is noise, no grading on a curve). `_REVIEW_EXAMPLE` shows good vs bad assessments in user message.

### Pipeline: caption → generate → evaluate
- `analyze.py` - Zero-step: parallel Gemini vision + reasoning-model style analysis → `StyleProfile` + initial `PromptTemplate` seeds. Cached via `runs/.cache/style_{hash}.json`. Bootstrap captions use a generic section format; the compilation prompt explicitly notes this so the template defines its own section names. Compilation prompt includes a watercolor-landscape JSON example for structural reference without content anchoring.
- `caption.py` - Gemini Pro captioning with disk cache. Validates captions are non-empty and ≥ 150 chars. `CAPTION_SYSTEM` emphasizes text-to-image specificity: precise color names, medium-appropriate terminology, spatial quantification. Zero-step `CAPTION_PROMPT` uses fixed sections with Subjects as most important, specific color names, and 200-400 word target.
...
- `evaluate.py` - Per-image paired metrics dispatched via `asyncio.to_thread`, Gemini/xAI vision comparison, and caption compliance. `_VISION_SYSTEM` includes calibration examples for MATCH/PARTIAL/MISS across all three dimensions. `_PAIRWISE_COMPARE_PROMPT` requests per-aspect verdicts (style, color, subject, composition) alongside overall winner; `_parse_pairwise_response` (module-level) extracts winner + rationale for both provider branches. `VERDICT_PATTERN` is the canonical regex consumed by `reporting/render.py`. `compute_caption_compliance` parses each caption once and returns `(CaptionComplianceStats, prose)` — `check_caption_compliance` and `compute_caption_compliance_stats` are thin wrappers. `compute_style_consistency` uses `parse_labeled_sections` for the `[Art Style]` block. `evaluate_images` returns `(scores, n_eval_failed)` with zero-score sentinels preserving index alignment; callers derive their own `completion_rate`.
...
- `reasoning_client.py` - Provider-agnostic `ReasoningClient` supporting Anthropic / OpenAI / xAI / Z.AI / local (OpenAI-compatible) backends; streaming + text-extraction helpers. Applies lightweight provider-specific prompt adaptations via `_adapt_prompts_for_provider` (GPT gets a user-message reminder for critical rules; GLM gets JSON-formatting reinforcement; others pass through unchanged).
...
- Persisted collections (`experiment_history`, etc.) must be bounded — cap and drop oldest entries rather than growing without limit in state.json. `experiment_history` is capped at `_MAX_PERSISTED_HISTORY = 10` (in `workflow/iteration_persistence.py`); history entries are written with `iteration_captions=[]` and `rendered_prompt=""` since the full payload survives in `{log_dir}/iter_NNN_branch_M.json` per-iter logs. Confirmatory replicates of the incumbent use `_INCUMBENT_BRANCH_ID = 900` (in `workflow/iteration_execution.py`) so they never collide with proposal indices.
...
- Meta-prompt is 2000-8000 words with 8-20 sections. `[Art Style]` and `[Subject]` caption blocks are expected to run roughly 1000-2000 words each; ancillary caption sections typically 150-400 words. The FIRST section must be `style_foundation` (fixed style rules from StyleProfile) and the SECOND section must be `subject_anchor` (detailed subject-fidelity instructions). The first two caption output labels must be `[Art Style]` and `[Subject]`. These required anchors are enforced by `_REQUIRED_SECTION_ANCHORS` / `_REQUIRED_CAPTION_ANCHORS` tables in `prompt/_parse.py`.
...
- `ReasoningClient.call` and `.call_json` accept optional `temperature` and `reasoning_effort` kwargs. Per-stage defaults: brainstorm (temp 0.9, effort "high"), rank (0.1, "low"), expand (0.3, "high"), synthesis (0.3, "medium"), review (0.2, "high"), zero-step analysis and compilation (0.3, "medium"). Provider mapping: Anthropic maps `reasoning_effort` to `thinking={"type": ...}` blocks and only applies custom temperature when effort is "low" (extended thinking forces temp=1); OpenAI threads both via `responses.create` (`reasoning.effort` + `temperature`); Z.AI/xAI/local accept temperature when provided and ignore reasoning_effort.
...
- Caption cache keys are content-derived (`p{sha256(meta_prompt)[:12]}`) in both protocol modes — identical meta-prompts share cache entries across experiments and iterations. The old `iter{N}_e{E}` key format is gone.
