...
- `prompt/experiments.py` - Per-iteration `propose_experiments` (single JSON batch of 8-12 raw proposals grouped into 3 directions) + `enforce_hypothesis_diversity` (drops duplicates keyed on `(category, failure_mechanism, intervention_type)`) + `select_experiment_portfolio` (picks 1 targeted per direction in D1→D2→D3 order, then fills remaining slots with bolds up to `num_branches`).
