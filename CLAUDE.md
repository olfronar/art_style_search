...
- `report.py` - Post-run HTML report generator. `build_report(run_dir)` writes `runs/<name>/report.html` (Plotly metric trajectories, per-iteration drill-down, image comparison grid, hypothesis tree + KB state). Plotly is lazy-imported so the `list`/`clean` commands stay fast. Images use relative paths via `_rel(target, report_dir)`. CSS lives in `reporting/report.css` (loaded via `importlib.resources`). `--offline` flag embeds Plotly JS inline for offline viewing.
