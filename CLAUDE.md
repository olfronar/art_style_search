...
- `runs/` - All run data, each run in its own subdirectory (not committed):
  ...
  - `runs/<name>/logs/` - Iteration logs (`iter_NNN_branch_M.json`), proposal fate logs (`iter_NNN_proposals.json` — every raw sketch the reasoner emitted plus its fate: brainstormed → trimmed / deduped_stage1 / deduped_stage2 / not_picked / invalid_template / executed; feeds the Hypothesis Tree report tab), captions cache, style profile, and the best meta-prompt in three parallel forms: `best_prompt.txt` (legacy flat string), `best_prompt.md` (structured markdown with YAML front-matter — iter/score/seed/sha), `best_prompt.json` (full `PromptTemplate` dataclass dump, re-ingestable as seed)
...
- **MegaStyle** (11%): Cosine similarity between ref and gen in a SigLIP SoViT-400M embedding fine-tuned on 1.4M style-paired images ([MegaStyle, Gao et al. 2026, arXiv:2604.08364](https://arxiv.org/abs/2604.08364)). Continuous, content-disentangled style-space signal — Spearman ~0 with DreamSim (content) and the ternary vision judge (Gemini) on a 754-pair calibration sweep, so it adds a new independent axis. Higher = better. Weight history: entered the composite at 5% funded by demoting `style_consistency` (see below); promoted 5% → 8% after the homescapes run showed the ternary `vision_style` judge systematically demoting branches with the highest MegaStyle — two nominal "style" signals anti-correlating in practice; promoted 8% → 11% on 2026-04-23 after the homescapes-run evaluation confirmed canon-verbatim-paste makes `style_consistency` a near-constant, absorbing its remaining 3%. MegaStyle is continuous and cheap, `vision_style` is coarse and costly, so MegaStyle is the primary style-similarity weight with `vision_style` in a secondary regression-alarm role.
...
- **Vision style** (3%): Per-image Gemini ternary comparison of style fidelity (MATCH=1.0, PARTIAL=0.5, MISS=0.0). Higher = better. Demoted 6% → 3% when MegaStyle was promoted (see MegaStyle above); at this weight it behaves as a regression alarm against MegaStyle's continuous style signal.
- **Style consistency** (0%, diagnostic only): Jaccard word-overlap of [Art Style] blocks across captions (experiment-level, omitted from `per_image_composite`). Weight history: 8% → 3% on the MegaStyle addition, then 3% → 0% on 2026-04-23 after the homescapes-run evaluation showed the captioner-contract-enforced canon-verbatim-paste makes this metric a near-constant when the contract holds — canon pull-through is already measured by `style_canon_fidelity` (LCS ratio) + `observation_boilerplate_purity` (trigram containment) inside the compliance penalty. Still computed and surfaced in KB `metric_deltas` as a regression diagnostic; no longer rendered in the HTML report, no longer contributes to `composite_score` / `headroom_composite_score` / `per_image_composite` / `adaptive_composite_score`, and no longer referenced in reasoner-facing prompts (experiments / synthesis / review).
...
`per_image_composite` uses the same base weights minus `_W_STYLE_CON` (experiment-level only, and since 2026-04-23 0-weight regardless) and applies no penalties — max output 1.00.
