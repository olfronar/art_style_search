...
- `prompt/experiments.py` - Per-iteration `propose_experiments` (N `<branch>` tags in one call) + `enforce_hypothesis_diversity` (drops duplicate-`target_category` branches).
- `prompt/synthesis.py` - `synthesize_templates`: merges top 2-3 experiments' best sections into a single template.
- `prompt/review.py` - CycleResearcher-inspired `review_iteration` (skeptical reviewer → `strategic_guidance`).

### Pipeline: caption → generate → evaluate
- `analyze.py` - Zero-step: parallel Gemini vision + reasoning-model style analysis → `StyleProfile` + initial `PromptTemplate` seeds. Cached via `runs/.cache/style_{hash}.json`.
- `caption.py` - Gemini Pro captioning with disk cache. Validates captions are non-empty and ≥ 150 chars.
...
- `state_migrations.py` - Schema version constants (`_SCHEMA_VERSION=3`, manifest=3, iteration-log=1, promotion-log=1) and payload migrators (notably `dino_similarity` → `dreamsim_similarity`).
...
- Hypothesis variability is enforced at 3 layers: (1) **Prompt-level** — each `<branch>` requires a `<target_category>` tag unique across branches; (2) **Post-parse dedup** — `enforce_hypothesis_diversity()` in `prompt/experiments.py` uses the parsed `target_category` from `RefinementResult` (falling back to `classify_hypothesis()` keyword matching if the tag is missing) to drop duplicate-category experiments; (3) **KB-guided targeting** — `suggest_target_categories()` (in `prompt/_format.py`) ranks categories by improvement potential (unexplored=1.0, partial success=0.7, diminishing returns=0.1) and injects the ranked list into the user message. The dedup filter is called in `workflow/iteration_proposals.py` after `propose_experiments()` returns.
