...
Treat model-facing prompts as product logic, not incidental strings. When editing `prompt/*.py`, `analyze.py`, `caption.py`, or evaluation/review prompts, preserve the local structure the repo now uses: ordered priority or phased sections, explicit execution checklists, and short `Why:` rationales where a constraint is easy for the model to ignore. Keep prompt text, JSON contracts in `prompt/json_contracts.py`, structural validation in `prompt/_parse.py`, and any prompt-specific parsing assumptions in sync.

## Testing Guidelines
Write pytest coverage beside the affected area using `tests/test_<module>.py`. Follow existing patterns with `tmp_path`, `monkeypatch`, and fake inputs instead of real API calls. When changing proposal generation, direction grouping, or portfolio selection, update `tests/test_prompt.py`, `tests/test_diversity.py`, `tests/test_iteration_proposals.py`, and the loop-level behavior in `tests/test_loop_e2e.py`. When changing model-facing prompt text or prompt-contract assumptions in `analyze.py`, `caption.py`, `evaluate.py`, or `prompt/*.py`, update the focused parser/compliance coverage in `tests/test_analyze.py`, `tests/test_prompt.py`, `tests/test_evaluate.py`, and `tests/test_evaluate_compliance.py`; include `tests/test_loop_e2e.py` when the workflow-visible contract changes. When changing persistence, hypothesis metadata, or historical run artifacts, update round-trip and migration coverage in `tests/test_state.py` and behavior coverage in `tests/test_knowledge.py`. When changing report structure or HTML rendering, update `tests/test_report.py`. When changing rigorous-mode promotion or scoring logic, cover the degenerate cases in `tests/test_scoring_rigor.py` and the policy-level behavior in `tests/test_loop.py`. Run targeted tests during iteration, then `uv run pytest tests/` and `uv run ruff check .` before wrapping up.
...
Recent history uses short, imperative commit subjects such as `Improve prompt quality across all model-facing prompts` and `Dedupe compliance checks and refresh docs`. Keep commits focused and descriptive. PRs should summarize behavior changes, list test coverage, note any CLI, config, or schema updates, and include screenshots when changing `report.html` output.
